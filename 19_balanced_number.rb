...
def sum_array(arr,acc=0)
  arr.length == 0 ? acc : sum_array(arr.drop(1),acc += arr[0]) 
end

def balanced_num(number)
  number = number.to_s.chars.map { |x| x.to_i }
  offset = number.length % 2 == 0 ? 1 : 0
  sym = (number.length/ 2.0).floor - offset
  sum_array(number[0..(sym - 1)]) == sum_array(number[-sym..-1]) ? "Balanced" : "Not Balanced"
end
